...
import { Save, Database, Key, Trash2, CheckCircle, AlertTriangle, Cpu, ExternalLink } from 'lucide-react';
...
const SettingsScreen: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-10">
      ...
      {/* SECTION 1: AI SETTINGS (PRIORITY) */}
      <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md border-l-4 border-emerald-500">
        <h2 className="text-xl font-bold mb-4 dark:text-white flex items-center gap-2">
          <Key className="w-6 h-6 text-emerald-600" /> 
          Chave de Inteligência (Google Gemini)
        </h2>
        
        <div className="mb-6 bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800">
           <p className="text-sm text-blue-800 dark:text-blue-200 mb-2">
             Para a Mara funcionar, você precisa de uma chave gratuita do Google.
           </p>
           <a 
             href="https://aistudio.google.com/app/apikey" 
             target="_blank" 
             rel="noreferrer"
             className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm font-semibold transition"
           >
             <ExternalLink className="w-4 h-4" /> Gerar Chave Gratuita Agora
           </a>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
               Recomendado: <code>gemini-2.0-flash</code> ou <code>gemini-1.5-flash</code>
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-end pt-4">
            <button onClick={handleSaveApi} className="bg-emerald-600 text-white px-6 py-2 rounded hover:bg-emerald-700 flex items-center gap-2 text-sm font-medium shadow-sm">
              <Save className="w-4 h-4" /> Salvar e Ativar IA
            </button>
        </div>
      </div>
